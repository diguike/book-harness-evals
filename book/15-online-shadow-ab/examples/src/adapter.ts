...
// 本文件是全书 adapter 接口的 canonical 定义源（第 5 章定义），各章示例都 import 它。
...
/** agent 主动停下来问人 / 升级的事件（第 13 章 Ask-F1 读 kind） */
...
// 本章把每次工具调用规整成一个 action 节点串成链，第 8 章做成完整 DAG。
...
/** 构造变体时的补丁：关掉哪些模块、替换哪些模块（影子里用它把写工具换成 dry-run） */
