...
// 每次 run 独立一份，工具读写这份隔离副本，并发回放时互不串台（第 7 章要同时跑几十个任务）。
...
export class StepRecorder {
  ...
  /** 记一条 ask / 升级事件，stepId 关联到刚记下的那一步（第 13 章评 Ask-F1 用） */
  recordAsk(kind: AskEvent['kind'], question: string, stepId?: string): void {
    this.askEvents.push({
  ...
  toOtar(): OtarNode[] {
    ...
    for (const s of this.steps) {
      ...
      nodes.push({
        ...
        module: s.action, // 本章里工具 id 即模块 id，归因时用得上
        ts: s.ts,
      });
      prevId = id;
    }
